...
void show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize, cv::Size imageSize, bool bWait)
{
    ...
    cv::namedWindow(windowName, 1); //cv::WINDOW_NORMAL); 
...
void save_show3DObjects(std::vector<BoundingBox> &boundingBoxes, cv::Size worldSize, cv::Size imageSize, bool bWait, bool bSave, std::string saveImgFullFilename)
{
    // create topview image
    cv::Mat topviewImg(imageSize, CV_8UC3, cv::Scalar(255, 255, 255));
  	std::string path;
    ...
    cv::imshow(windowName, topviewImg);
  	...
  	path = saveImgFullFilename + "_topviewImg" + ".png";
  	if(bSave)
      cv::imwrite(path, topviewImg);
}